...
\brief  
  This file contains functions for using the C# Reflection System in order to obtain values from
  various information at runtime. The code was taken from a personal code library project known
  as SINGULARITY (Previously called ENTITY).
...
{
  /// <summary>
  /// A series of handy functions for using the C# Reflection System. These can be used both at runtime, and in
  /// Unity's Editor GUI code.
  /// </summary>
  public static partial class Reflection
  {
    /// <summary> The character used by Unity to separate individual pieces of a full member path. </summary>
    private static readonly char PathSeparator = '.';
    /// <summary> The keyword declaring that a path is going to access an element of an IList. </summary>
    private static readonly string ArrayKeyword = "Array";
    /// <summary> The keyword right before the index of a path in an IList element. </summary>
    private static readonly string DataKeyword = "data[";
    /// <summary> The string used to tell an Attribute to find a variable. </summary>
    public static readonly string VariableCall = "!";
    ...
    public static BindingFlags DefaultFlags { get { return BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static | BindingFlags.FlattenHierarchy; } }

    /// <summary>
    /// A function for getting the value of a dynamically accessed field. This is useful for PropertyDrawers.
    /// </summary>
    /// <typeparam name="T">The type stored in the wanted value.</typeparam>
    /// <param name="obj">The starting object to search from.</param>
    /// <param name="path">The path to the value.</param>
    /// <param name="default_return">The default value to return should the search fail.</param>
    /// <param name="found_value">The found value at the path.</param>
    /// <returns>Returns if the wanted value was successfully found.</returns>
    public static bool CheckDynamicField<T>(object obj, string path, T default_return, out T found_value)
    {
      found_value = default_return; // Initialize the found value.

      // Check that nothing is empty, and that we are searching for a variable call. 
      if (!string.IsNullOrWhiteSpace(path) && path.First().ToString() == VariableCall)
      {
        string sub_path = path.Substring(1); // Get the substring without the variable call character.

        // Get the wanted field info.
        object current = obj;
        FieldInfo info = GetFieldInfo(ref current, out object previous, DefaultFlags, sub_path.Split('.'));

        // If the field info is found, check if it is of the wanted type. If so, return it.
        if (info != null)
        {
          object value = info.GetValue(previous);
          if (value is T)
          {
            found_value = (T)value;
            return true;
        ...
        current = obj;
        PropertyInfo pinfo = GetPropertyInfo(ref current, out previous, DefaultFlags, sub_path.Split('.'));

        if (pinfo != null)
        {
          object value = pinfo.GetValue(previous);
          if (value is T)
          {
            found_value = (T)value;
            return true;
      ...
      if (field != null)
      {
        ...
        switch (previous)
        {
          ...
          case IList ilist when path.LastElement().Contains(DataKeyword):
            ...
            if (ilist != null && ilist.IsValidIndexGeneric(index))
            {
          ...
          default:
            ...
            return field.GetValue(previous) == (object)value; // Return if the value was correctly set.
        }
      }
          ...
          switch (current)
          {
            ...
            case IList ilist:
              ...
              if (path[i] == ArrayKeyword)
              {
                ...
                current = ilist.IsValidIndexGeneric(index) ? ilist[index] : null; // If the index is valid, return that object. Else, return null;
              }
              break;
            default:
